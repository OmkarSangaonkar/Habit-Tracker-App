...
export function habitsReducer(state = initialHabitsState, action) {
  let updatedList; // Declare variable outside the switch statement
  ...
  switch (action.type) {
    ...
    case ADD_HABIT_STATUS: {
      updatedList = state.list.map((elem) =>
        elem.id === action.id
    ...
    case ADD_WEEKLY_DONE_STATUS: {
      let updatedList2 = state.list.map((elem) =>
        elem.id === action.id
          ? {
              ...elem,
              weekdays: elem.weekdays.map((item, index) =>
                index === action.index
